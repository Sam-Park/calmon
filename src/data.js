// 1. Box Breathing                     Games     
// 2.Calm Counting                     Games
// 3. Decoding Messages           Games
// 4. Going on an Adventure       Games
// 5. Match the Color                   Games
// 6. Superhero Workout             Games
// 7. 5-4-3-2-1 Technique             Games
// 8. Healthy Habits                     Informational
// 9. How to do the Five              Informational
// 10. How to wash your hands   Informational
// 11. Milk, Milk, Milk                     Games
// 12. Hokey Pokey                       Video
// 13. Jumping Jacks                   Informational
// 14. Pushups                            Informational
// 15. What is COVID-19?           Video

const activities = [{name:'Box Breathing', type:'Games', id: 1}, 
                {name:'Calm Counting', type:'Games', id: 2},
                {name:'Decoding Messages ', type:'Games', id: 3}, 
                {name:'Going on an Adventure', type:'Games', id: 4}, 
                {name:'Match the Color', type:'Games', id: 5}, 
                {name:'Superhero Workout ', type:'Games', id: 6}, 
                {name:'5-4-3-2-1 Technique', type:'Games', id: 7}, 
                {name:'Healthy Habits ', type:'Informational', id: 8}, 
                {name:'How to do the Five ', type:'Informational', id: 9}, 
                {name:'How to wash your hands', type:'Informational', id: 10}, 
                {name:'Milk, Milk, Milk', type:'Games', id: 11}, 
                {name:'Hokey Pokey ', type:'Video', id: 12}, 
                {name:'Jumping Jacks ', type:'Informational', id: 13}, 
                {name:'Pushups', type:'Informational', id: 14}, 
                {name:'What is COVID-19?', type:'Video', id: 15}, 
            ]

export default activities